...
def tiff2png(path_to_files,path_to_save):
    ...
    for file in glob(path_to_files+'/*.tiff'):
        ...
        img = _stretch_im(img,5)
        # img = img.astype(np.float32) / np.iinfo(img.dtype).max
        img = np.transpose(img,(1,2,0))
        print(file_name)
        print(img.shape[-1])
        if img.shape[-1] == 2 :
            imwrite(path_to_save+file_name+'_band1.png',img[:,:,0].astype(np.uint16))
            imwrite(path_to_save+file_name+'_band2.png',img[:,:,1].astype(np.uint16))
        else :
            imwrite(path_to_save+file_name+'_p1.png',img[:,:,:3].astype(np.uint16))
            if img.shape[-1] == 6 :
                imwrite(path_to_save+file_name+'_p2.png',img[:,:,3:].astype(np.uint16))
            else :
                imwrite(path_to_save+file_name+'_p2.png',img[:,:,3].astype(np.uint16))
...
def tiff2jpeg(path_to_files,path_to_save):
    ...
    for file in glob(path_to_files+'/*.tiff'):
        ...
        # img = beautify(img)
        img = img.astype(np.float32) / np.iinfo(img.dtype).max
        # img = _stretch_im(img,str_clip=1)
        img = np.transpose(img,(1,2,0))
        print(file_name)
        print(img.shape[-1])
        if img.shape[-1] == 2 :
            imwrite(path_to_save+file_name+'_band1.jp2',img[:,:,0].astype(np.float32))
            imwrite(path_to_save+file_name+'_band2.jp2',img[:,:,1].astype(np.float32))
        else :
            imwrite(path_to_save+file_name+'_p1.jp2',img[:,:,:3].astype(np.float32))
            if img.shape[-1] == 6 :
                imwrite(path_to_save+file_name+'_p2.jp2',img[:,:,3:].astype(np.float32))
            else :
                imwrite(path_to_save+file_name+'_p2.jp2',img[:,:,3].astype(np.float32))

def save_image(img,channels,path,driver='GTiff'):
    with rio.open(
        path+'.tif',
        'w',
        driver=driver,
...
def get_patches(path_to_directory,type=None,paths_to_imgs=False,imgs=True):
    if path_to_directory[-1]=='/':
        path_to_directory = path_to_directory[:-1]
    path_dict = {}
    img_dict = {}
    if type==None:
        full_path = path_to_directory+'/*.tiff'
    else :
        full_path = path_to_directory+f'/*{type}.tiff'
    for file in glob(full_path):
        if paths_to_imgs:
            if file.split('_')[-2].split('/')[-1] in path_dict:
                path_dict[file.split('_')[-2].split('/')[-1]].append(file)
            else : 
                path_dict[file.split('_')[-2].split('/')[-1]] = [file]
        if imgs:
            if file.split('_')[-2].split('/')[-1] in img_dict:
                with rio.open(file,'r') as f:
                    img = f.read()
                    img = img.astype(np.float32) / np.iinfo(img.dtype).max
                    img_dict[file.split('_')[-2].split('/')[-1]].append(img)
            else : 
                with rio.open(file,'r') as f:
                    img = f.read()
                    img = img.astype(np.float32) / np.iinfo(img.dtype).max
                    img_dict[file.split('_')[-2].split('/')[-1]] = [img]
...
def reconstruct_image(path_to_patches=None,patches=None,type=None):
    # Get patches in a dict 
    if path_to_patches :
        patches = get_patches(path_to_patches,type=type)
    ...
    while patch <=len(patches):
        if patch == 1 :
            tmp = patches[f'patch{patch}'][0]
            patch+=1
            continue
        
        if patch % ppd == 0 :
            tmp = np.dstack((tmp,patches[f'patch{patch}'][0]))
            rows.append(tmp)
