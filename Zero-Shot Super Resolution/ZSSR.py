...
class ZSSR:
    ...
    def __init__(self, input_img, conf=Config(), ground_truth=None, kernels=None):
        ...
        # elif self.input.shape[-1]==6 :
        #     self.input = self.input[:,:,:3]
            # self.input2 = self.input[:,:,3:]   
        # else :
        #     self.input=None

        # Acquire meta parameters configuration from configuration class as a class variable
        self.conf = conf
        self.filter_shape = ([[self.input.shape[-1], self.input.shape[-1], self.input.shape[-1], self.conf.width]] +
                             [[self.input.shape[-1], self.input.shape[-1], self.conf.width, self.conf.width]] * (self.conf.depth-2) +
                             [[self.input.shape[-1], self.input.shape[-1], self.conf.width, self.input.shape[-1]]])
    ...
    def run(self):
        ...
        for self.sf_ind, (sf, self.kernel) in enumerate(zip(self.conf.scale_factors, self.kernels)):
            ...
            if self.conf.save_results:
              ...
              np.save('%s/%s_zssr_%s.npy' %(self.conf.result_path, os.path.basename(self.file_name)[:-4], sf_str),post_processed_output)
              # self.save_image(post_processed_output)
                
                # imwrite('%s/%s_zssr_%s.png' %
                #            (self.conf.result_path, os.path.basename(self.file_name)[:-4], sf_str),
                #            post_processed_output.astype(np.uint16))
    ...
    def build_network(self, meta):
        with self.model.as_default():
            ...
            # Filters
            self.filters_t = [tf.get_variable(shape=self.filter_shape[ind], name='filter_%d' % ind,
                                              initializer=tf.random_normal_initializer(
                                                  stddev=np.sqrt(meta.init_variance/np.prod(
                                                      self.filter_shape[ind][0:3]))))
