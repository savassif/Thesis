...
def random_augment(ims,
    ...
    if im.shape[-1]<=4:
      ...
      b1_3 = np.clip(warpPerspective(im[:,:,:3], transform_mat, (crop_size, crop_size), flags=INTER_CUBIC), 0, 1)
      b4_6 = np.clip(warpPerspective(im[:,:,3:], transform_mat, (crop_size, crop_size), flags=INTER_CUBIC), 0, 1)
      return np.dstack((b1_3,b4_6))
